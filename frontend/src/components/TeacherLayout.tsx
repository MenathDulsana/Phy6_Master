...
import {
  ...
  GraduationCap, LogOut, ChevronDown, Bell, FileQuestion,
  Menu, X, User,
} from "lucide-react";
...
export default function TeacherLayout() {
  ...
  const userName = localStorage.getItem("authName") || "Teacher";
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 w-60 flex flex-col border-r border-border bg-sidebar transition-transform duration-300
        ...
        <div className="flex items-center justify-between px-4 py-5 border-b border-sidebar-border">
          <div className="flex items-center gap-2">
            ...
            <span className="font-display text-lg font-bold text-sidebar-foreground">Teacher</span>
          </div>
          ...
          {sidebarLinks.map((link) => {
            ...
            return (
              <Link key={link.path} to={link.path}
                className={`flex items-center gap-3 rounded-lg border-l-4 py-2.5 pl-2 pr-3 text-sm font-medium transition-colors
                  ${active
                    ? "border-amber-400 bg-sidebar-accent text-sidebar-primary"
              ...
              <AnimatePresence>
                {profileOpen && (
                  <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 5 }}
                    ...
                    <Link to="/teacher/dashboard" onClick={() => setProfileOpen(false)} className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-foreground hover:bg-accent">
